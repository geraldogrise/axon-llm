"""Build the Docker specialist: one expert per FAMILY.
...
treinamento_docker/ (local repo, or the sibling `treinamento` data repo).
Saves the modular router + a sparse semantic KB to axon_lang_data/docker_experts/.

Env: AXON_DOCKER_DIR (data dir), AXON_EPOCHS (300), AXON_BATCH (256), AXON_LSA_DIM (200).
"""
...
_LOCAL = os.path.join(ROOT, "treinamento_docker")
_DATA = os.path.join(ROOT, "..", "treinamento", "treinamento_docker")
DATA_DIR = os.environ.get("AXON_DOCKER_DIR", _LOCAL if os.path.isdir(_LOCAL) else _DATA)
OUT = os.path.join(HERE, "axon_lang_data", "docker_experts")
...
QUESTIONS = [
    ('fundamentos', 'qual a diferenca entre containers e maquinas virtuais?'),
    ('fundamentos', 'o que sao imagens e containers e o docker engine?'),
    ('imagens', 'como escrever um dockerfile e suas instrucoes?'),
    ('imagens', 'como funcionam as camadas e o cache no build de imagens?'),
    ('imagens', 'como escanear vulnerabilidades e publicar em registries privados?'),
    ('runtime-dados', 'como usar volumes para persistir dados no docker?'),
    ('runtime-dados', 'como gerenciar secrets e observabilidade em producao?'),
    ('redes', 'como funcionam as redes e a descoberta por nome no docker?'),
    ('compose', 'como orquestrar multiplos containers com docker compose?'),
]
...
def main():
    ...
    print("docker: %d lessons | families: %s" % (len(docs), per_family), flush=True)
